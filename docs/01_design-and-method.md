...
- 스키마·합성 방법론·문서 작성: DATA (Gonnector AI Team)

> 본 문서의 설계 결정은 Dylan과 DATA의 협업 산출물이다. Dylan은 효돌 도메인 이해 · 표본 분포 도메인 조정 · 사용 패턴 7종 정의 · 옵션 B 머지 전략 · 외부 평가 sub-agent 도입 · 분모 정의 명확화 등 **전략·도메인 영역의 핵심 결정**을 제공했고, DATA는 그 결정 위에서 스키마 설계 · 잠재 변수 합성 모델 · 인지 측정 페어링 메커니즘 · 합성 코드 구현 · 검증 워크플로우 등 **기술적 실행 영역**을 담당했다.
